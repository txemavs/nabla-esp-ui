...
Negative values restore the route-specific idle footer state. Values are clamped to -2..100.
There is no header divider or header progress indicator anymore.
toolbar_demo_load runs twenty 50 ms steps and restores the route-specific idle state at completion.
