...
struct CompactWifi {
  ...
  bool editing=false, secret=false;
  ...
  void move(int delta) {
    ...
    focus=nabla::wrap_focus(focus,delta,total());
    top=nabla::scroll_anchor(focus,top,3,total());
  }
  ...
  bool activate(uint32_t now) {
    ...
    if(flow.stage==Stage::RESULTS) {
      if(focus<flow.results())flow.choose(focus);else flow.cancel();
