...
The status correctly distinguishes timeout from sample consumption; consumption
does not prove audible playback without a physical listen test.
...
for a CVSD diagnostic baseline. HFP client support remains compiled as a
service-record-capacity workaround, but only AG is initialized.
