...
from .cache import CameraCache, Unavailable

DOMAIN = "nabla_camera_cache"

CONFIG_SCHEMA = vol.Schema({
    DOMAIN: vol.Schema({
        vol.Required("cameras"): vol.All(
            cv.ensure_list, [cv.entity_domain("camera")],
            vol.Length(min=1, max=32), lambda items: list(dict.fromkeys(items))),
        vol.Optional("refresh_interval", default=1.0):
            vol.All(vol.Coerce(float), vol.Range(min=0.1, max=60)),
        vol.Optional("max_stale", default=8.0):
            vol.All(vol.Coerce(float), vol.Range(min=1, max=60)),
        vol.Optional("quality", default=70):
            vol.All(vol.Coerce(int), vol.Range(min=30, max=85)),
    })
...
async def async_setup(hass, config):
    ...
    for entity_id in options["cameras"]:
        async def fetch(entity=entity_id):
            image = await async_get_image(hass, entity, timeout=8)
            return image.content

        caches[entity_id] = CameraCache(
            fetch, hass.async_add_executor_job, gate,
            interval=options["refresh_interval"], max_stale=options["max_stale"],
            quality=options["quality"])
    hass.data[DOMAIN] = caches
    hass.http.register_view(CachedImageView(hass.data[DATA_COMPONENT], caches))

    async def stop(_event):
        await asyncio.gather(*(cache.close() for cache in caches.values()))
