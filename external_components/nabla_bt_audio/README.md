...
The latest status fix distinguishes timeout from sample consumption; consumption
does not prove audible playback. This fix has compile/host evidence only.

Host and controller select HCI. Wideband is disabled for a CVSD diagnostic
baseline. HFP client support remains compiled as a service-record-capacity
workaround, but only AG is initialized. Investigate before production use.
