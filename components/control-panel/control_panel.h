...
inline int columns(int screen_w) { return screen_w > 320 ? 4 : (screen_w > 200 ? 3 : 2); }
...
inline void render_cell(lv_obj_t* btn, lv_obj_t* label, const Cell& cell, const Words& words,
                        bool dark, bool focused, int width, int height) {
  ...
  if (cell.kind == CellKind::Sensor) {
    if (cell.available) {
      text += std::to_string(cell.value);
      text += cell.unit;
    } else {
      text += words.unavailable;
    }
...
inline void render_tabs(lv_obj_t* container, const std::vector<Section>& sections,
                        int current, bool dark, int screen_w) {
  ...
  int tab_width = std::min(120, (screen_w - 20) / (int)sections.size());
  int x = 10;
  for (size_t i = 0; i < sections.size(); i++) {
    lv_obj_t* btn = lv_button_create(container);
    lv_obj_set_size(btn, tab_width, 28);
    ...
    lv_obj_set_style_border_color(btn, lv_color_hex(dark ? 0xFFFFFF : 0x000000), 0);
...
inline void render_sensor_row(lv_obj_t* container, const std::vector<Cell>& sensors,
                              const Words& words, bool dark, int y, int screen_w) {
  if (sensors.empty()) return;
  int cell_w = (screen_w - 20) / std::max(1, (int)sensors.size());
  ...
  for (const auto& cell : sensors) {
    ...
    lv_obj_set_size(box, cell_w - 4, 28);
